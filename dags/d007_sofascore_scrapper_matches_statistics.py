...
def dag_sofascore_scrapper_07_matches_statistics():
    @task
    ...
    disparar_proxima_dag = TriggerDagRunOperator(
        task_id='trigger_sofascore_scrapper_08_lineups',
        trigger_dag_id='sofascore_scrapper_08_lineups',
         conf={},
    )

    obter_seasons >> consolidar_seasons >> obter_rounds_slugs >> consolidar_rounds_slugs >> obter_matches >> consolidar_matches >>\
    verificacao >> extracao >> transformacao >> disparar_proxima_dag

dag_sofascore_scrapper_07_matches_statistics()  